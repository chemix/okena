...
// `ShellType` itself is pure data and lives in `okena-core` so data-only crates
// can use it without the PTY stack. Re-exported here so existing
// `okena_terminal::shell_config::ShellType` paths keep working.
pub use okena_core::shell::ShellType;

/// Build a spawnable `CommandBuilder` from a [`ShellType`].
///
/// This is the behavioral half of `ShellType` — kept in `okena-terminal`
